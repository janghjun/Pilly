...
export default function MedicationDetailScreen() {
  ...
  const filtered = useMemo(() => {
    const all = getMedicineByRange(format(startDate, 'yyyy-MM-dd'), format(endDate, 'yyyy-MM-dd'));
    return all.filter(m => m.category === tab);
  }, [getMedicineByRange, tab, currentDate, rangeTab]);

  const total = filtered.length;
  const taken = filtered.filter(m => m.checked).length;
  const percent = total ? Math.round((taken / total) * 100) : 0;

  const timeLabels = ['아침', '점심', '저녁', '자기전'];
  const timeStats = useMemo(() => {
    const timeMap = { 아침: 0, 점심: 0, 저녁: 0, 자기전: 0 };
    filtered.forEach(m => {
      if (m.time && timeMap[m.time] !== undefined) timeMap[m.time]++;
    });
    return timeLabels.map(label => timeMap[label]);
  }, [filtered]);
  ...
  const exportToCSV = async () => {
    ...
    csv += `\n요약,총 복약,복약 완료\n,${total},${taken}\n`;
    csv += `\n시간대별 통계,아침,점심,저녁,자기전\n,${timeStats.join(',')}`;

    const fileUri = FileSystem.documentDirectory + 'medication_summary.csv';
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.dateRow}>
          ...
          <Text style={styles.dateText}>{format(startDate, 'yyyy.MM.dd')} ~ {format(endDate, 'yyyy.MM.dd')}</Text>
          <TouchableOpacity onPress={() => moveDate(1)}>
            <Text style={styles.navBtn}>{'>'}</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.statLabel}>
            총 복약 <Text style={styles.strong}>{total}</Text>회 중 <Text style={styles.strong}>{taken}</Text>회 완료
          </Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  dateRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginBottom: 12, gap: 10 },
  tabButton: { paddingVertical: 6, paddingHorizontal: 16, backgroundColor: 'white', borderRadius: 8, marginRight: 8 },
  ...
  progressCard: { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 8 },
  percentText: { fontSize: 16, fontWeight: '600', color: '#3A70FF', width: 50, textAlign: 'right' },
  statLabel: { fontSize: 14, color: '#333' },
  strong: { fontWeight: '700', color: '#3A70FF' },
  medItem: { fontSize: 14, marginBottom: 6, color: '#444' },
  emptyText: { fontSize: 14, color: '#999' },
  csvBtn: { backgroundColor: '#2678E4', padding: 10, borderRadius: 6 },
