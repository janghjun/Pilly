...
export default function ExerciseDetailScreen() {
  ...
  if (!exercise) return <Text>운동 정보를 찾을 수 없습니다.</Text>;

  const filtered = useMemo(() => records.filter(r => r.name === exercise.name), [records, exercise]);
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.container}>
        <View style={styles.headerRow}>
          <TouchableOpacity onPress={() => navigation.navigate('ExerciseHome')}>
            <Text style={styles.backBtn}>{'<'}</Text>
          </TouchableOpacity>
          <Text style={styles.title}>운동 상세</Text>
        </View>

        <Image source={{ uri: exercise.thumbnail || 'https://via.placeholder.com/300x180' }} style={styles.image} />
          ...
          <View style={styles.card}>
            <View style={styles.titleRow}>
              ...
              <TouchableOpacity onPress={() => toggleFavorite(exercise.id)}>
                <Image
                  source={exercise.favorite ? require('../../assets/images/star_filled.png') : require('../../assets/images/star_empty.png')}
            ...
            <Text style={styles.desc}>
              이 운동은 {exercise.part} 부위를 강화하는 데 효과적인 운동입니다. 정확한 자세로 반복 수행하는 것이 중요합니다.
            </Text>
          </View>
        ) : (
          <View style={styles.card}>
        ...
        <TouchableOpacity style={styles.button} onPress={() => alert('루틴에 추가 완료')}>
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 16, fontWeight: '600', marginTop: 20, marginBottom: 10, color: '#111' },
  medItem: { fontSize: 14, marginBottom: 6, color: '#444' },
  emptyText: { fontSize: 14, color: '#999' },
  button: { backgroundColor: '#2678E4', padding: 16, borderRadius: 10, alignItems: 'center', marginTop: 10, marginBottom: 30 },
