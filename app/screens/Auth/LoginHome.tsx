...
export default function LoginHome({ navigation }: any) {
  ...
  const nextPage = () => {
    if (page < cardTexts.length - 1) {
      ...
      navigation.navigate('Login'); // 로그인 스크린으로 이동
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.card}>
        <Text style={styles.cardText}>{cardTexts[page]}</Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  nextButton: {
    position: 'absolute',
    bottom: 108, // 이전 버튼 기준 spacing 20px 위 (48 + 20 + 40 = 108)
    left: 22,
    width: 348,
    height: 48,
    borderRadius: 5,
  ...
  prevButton: {
    ...
    height: 48,
    borderRadius: 5,
    backgroundColor: '#F05636', // 이전 버튼 색상
    justifyContent: 'center',
    alignItems: 'center',
  },
  buttonText: {
